...
const TABLE = "ciiedo_agenda";

const TIPOS = ["Evento", "Taller", "Certificación"];
...
function CiiedoAgendaManager() {
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    if (!form.titulo.trim() || !form.fecha_texto.trim() || !form.tipo.trim()) return;
    setSaving(true);
    setError("");
  ...
  const handleDelete = async (item) => {
    if (!window.confirm(`¿Borrar "${item.titulo}" de la agenda? No se puede deshacer.`)) return;
  ...
  if (loading) return <p className="admpanel-loading">Cargando agenda…</p>;
    ...
    <div className="admpanel-section">
      ...
      {form && (
        <form className="admpanel-form" onSubmit={handleSave}>
          <h3>{form.id ? "Editar elemento" : "Nuevo elemento"}</h3>
          ...
          <div className="admpanel-field-row">
            ...
            <label className="admpanel-field">
              ...
              <input
                ...
                placeholder="Ej. 20 de agosto, 2026 · 10:00 hrs"
              />
            </label>
          </div>
          ...
          <label className="admpanel-field">
            ...
            <textarea
              ...
              placeholder="Sede, requisitos u otra información adicional."
            />
          </label>
      ...
      {items.length === 0 ? (
        <p className="admpanel-empty">Todavía no hay nada en la agenda. Crea el primero con "+ Nuevo elemento".</p>
      ) : (
...
export default CiiedoAgendaManager;
